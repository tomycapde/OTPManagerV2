...
namespace OTPManager.Shared.ViewModels
{
    public class CodesDisplayViewModel : MvxViewModel
    {
        ...
        private List<OTPDisplayViewModel> allGenerators = new List<OTPDisplayViewModel>();

        public bool HasGenerators => allGenerators.Any();

        public bool NoSearchResults => HasGenerators && !Items.Any();
        ...
        public IMvxCommand ClearSearch { get; }
            ...
            Import = new MvxCommand(async () =>
            {
                ...
                using (var inStream = await inFile.OpenAsync(System.IO.FileAccess.Read))
                {
                    ...
                    if (restoreSuccess)
                    {
                        ...
                        allGenerators = generators.Select(d => new OTPDisplayViewModel(d)).ToList();
                        RaisePropertyChanged(nameof(HasGenerators));
                        ApplyFilter();
                    }
                    else
                    {
        ...
        private void ApplyFilter()
        {
            ...
            else
            {
                ...
                Items = allGenerators
                    .Where(x => (x.Label != null && x.Label.IndexOf(query, StringComparison.CurrentCultureIgnoreCase) >= 0)
                             || (x.Issuer != null && x.Issuer.IndexOf(query, StringComparison.CurrentCultureIgnoreCase) >= 0))
                    .ToList();
            }
        }
        ...
        private void UIRefresh()
        {
            ...
            {
                foreach (var i in allGenerators)
                {
