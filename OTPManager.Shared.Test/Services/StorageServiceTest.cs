...
namespace OTPManager.Shared.Test.Services
{
    public class StorageServiceTest : TestBase<IStorageService>, IDisposable
    {
        ...
        [Fact]
        public async Task DumpRestoreWorksWithManyItems()
        {
            await Target.ClearAsync();
            var manyItems = Enumerable.Range(1, 100).Select(d => CreateOTPGenerator(d)).ToArray();
            var random = new Random();
            foreach (var i in manyItems)
            {
                random.NextBytes(i.Secret);
                await Target.InsertOrReplaceAsync(i);
            }

            var password = "encryptionPasswordWithManyItems";
            var dumpData = await Target.DumpAsync(password);

            var result = await Target.RestoreAsync(dumpData, password);
            Assert.True(result);

            var restoredItems = await Target.GetAllAsync();
            Assert.Equal(100, restoredItems.Count);
        }
    }
}
